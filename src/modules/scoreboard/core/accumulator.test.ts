...
describe('createHsAccumulator', () => {
  it('reports approximate with no rate before any own observation', () => {
    ...
    expect(acc.getDerived()).toEqual({ approximate: true, hsRatePercent: null });
  ...
  it('is exact from a fresh-match start and null until the first kill', () => {
    ...
    expect(acc.getDerived()).toEqual({ approximate: false, hsRatePercent: null });
  });
  ...
  it('includes the in-progress round in the rate immediately', () => {
    ...
    expect(acc.getDerived()).toEqual({ approximate: false, hsRatePercent: 50 });
  });
  ...
  it('folds completed rounds into the totals on round advance', () => {
    ...
    expect(acc.getDerived()).toEqual({ approximate: false, hsRatePercent: (2 / 3) * 100 });
  });
  ...
  it('folds a completed round exactly once under repeated payloads', () => {
    ...
    expect(acc.getDerived()).toEqual({ approximate: false, hsRatePercent: 100 });
  });
  ...
  it('rates 0 percent once kills exist without headshots', () => {
    ...
    expect(acc.getDerived()).toEqual({ approximate: false, hsRatePercent: 0 });
  });
